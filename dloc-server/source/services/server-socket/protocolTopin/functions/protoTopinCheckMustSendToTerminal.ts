...
const protoTopinCheckMustSendToTerminal = (
  ...
  const { uploadSec, heartBeatSec: statusPackageSec, forceReportLocInSec, ledState } = protoTopinGetPowerProfileConfig(newPowerProfile);
  ...
  printMessage(
    `${prefix} 📡 set status package [${statusPackageSec} sec], leds [${ledState}], Upload Interval [${uploadSec} sec], forceUpdateLoc [${forceReportLocInSec} sec]`
  );
