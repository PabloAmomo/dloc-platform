const distanceFromLatLngInMeters = (startLat: number, strarLng: number, endLat: number, endLng: number) => {
  ...
  const dLon = deg2rad(endLng - strarLng);
  const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(deg2rad(startLat)) * Math.cos(deg2rad(endLat)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
