...
const handlePacket = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  let lng: number =
    parseFloat(
      req.query?.lon?.toString() ?? req.query?.lng?.toString() ?? config.INVALID_POSITION_LAT_LNG.toString()
    ) ?? NaN;
  ...
  if (isNaN(lat) || isNaN(lng) || (lat === 90 && lng === 0)) {
    lat = config.INVALID_POSITION_LAT_LNG;
    lng = config.INVALID_POSITION_LAT_LNG;
  } else if (lat < -90 || lat > 90 || lng < -180 || lng > 180) error = "invalid latitude or longitude";
  ...
  protoHttpHandlePacket(getPersistence(), positionPacket).then((response) =>
