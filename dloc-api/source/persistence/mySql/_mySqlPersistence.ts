...
class mySqlPersistence implements Persistence {
  ...
  ): Promise<SaveDeviceParamsResult> {
  ...
  deleteShareDevice(
    imei: string,
    sharedImeiId: string
  ): Promise<DeleteShareDeviceResult> {
  ...
  ): Promise<AddShareDeviceCodeResult> {
  ...
  getPersistenceConfig(): string {
    ...
    return `host: ${host}, user: ${user}, database: ${database}, port: ${port}, password: ${password?.substring(
      0,
      3
    )}*********`;
  }
