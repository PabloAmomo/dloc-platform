...
const MapMarkerPopup = (props: MapMarkerPopupProps) => {
  const { device, zIndexOffset, onClick = () => {} }: MapMarkerPopupProps = props;
  const { openFormDevice } = useDevicesFormContext();
  const { t } = useTranslation();
  const { visiblePaths } = useMapContext();

  const datePosition: Date | undefined = convertUTCDateToLocalDate(device.lastPositionUTC);
  const dateVisibility: Date | undefined = convertUTCDateToLocalDate(device.lastVisibilityUTC);

  const datePositionText: string = datePosition ? formatDate(datePosition, t('dateString')) ?? '-' : '-';
  const dateVisibilityText: string = dateVisibility ? formatDate(dateVisibility, t('dateString')) ?? '-' : '-';
  ...
  const calculatDateVisibility = calculateTime(dateVisibility ?? undefined, t('calculateTime', { returnObjects: true }));
  const outOffVisibility = calculatDateVisibility.seconds > configApp.secondsOutOffVisibility;
  const outOffVisibilityBattery = calculatDateVisibility.seconds > (configApp.secondsOutOffVisibilityBattery);

  const distanceInMeters: number = visiblePaths.find((path: MapPath) => path.imei === device.imei)?.distance ?? 0;
  ...
  const LineItem = (sx: SxProps, label: string | JSX.Element, value: string | JSX.Element) => (
    <Box {...style.LineItem.ContainerProps} sx={{ ...style.LineItem.ContainerProps.sx, ...sx } as SxProps}>
      <Box {...style.LineItem.TitleSpanProps}>{label}</Box>
      <Box {...style.LineItem.ValueSpanProps}>{value}</Box>
    </Box>
  );

  return (
    <Box onClick={onClick} {...style.ContainerProps} style={zIndexOffset ? { zIndex: zIndexOffset } : {}}>
      {/* Has Image */}
      {device.params.hasImage && (
        <Box {...style.AvatarContainerProps}>
          <Avatar variant="rounded" alt="" src={getDeviceImagePathByImei(device.imei)} {...style.AvatarProps} />
        </Box>
      )}
      ...
      {LineItem(style.LastTimeSx, <LocationOnTwoTone {...style.ItemIconProp} />, <b>{`${datePositionText} ${calculatedTimePosition}`}</b>)}

      {/* Distance t('distance') */}
      {distanceInMeters > 0 && LineItem(style.DistanceSx, <ForkRightTwoTone {...style.ItemIconProp} />, distance)}

      {/* Speed t('avreagekm/h') */}
      {speedInKmH > 0 && LineItem(style.PositionSx, <SpeedIcon {...style.ItemIconProp} />, `${speedInKmH.toFixed(2)} ${t('avreagekm/h')}`)}

      {/* Last visibility  t('lastVisibility') */}
      {LineItem(style.LastVisibilityProps, <VisibilityTwoTone {...style.ItemIconProp} />, <>{`${dateVisibilityText} ${calculatedTimeVisibility}`}</>)}
      ...
      <Box {...style.BatteryIconContainerProps}>
        <BatteryIcon {...style.BatteryIconProps} level={outOffVisibilityBattery ? -1 : device?.batteryLevel ?? -1} />
      </Box>
      ...
      {device?.params?.sharedWiths && device?.params?.sharedWiths?.length > 0 && (
        <Box onClick={handleOpenConfig} {...style.SharedWithIconContainerProps}>
      ...
      <Box {...style.VisibilityIconContainerProps}>
        {outOffVisibility ? <WifiOff {...style.VisibilityIconProps.WithOutVisibility} /> : <Wifi {...style.VisibilityIconProps.WithVisibility} />}
      </Box>
    </Box>
  );
};
